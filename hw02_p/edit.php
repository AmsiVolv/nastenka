...
if(!empty($_REQUEST['removeId'])){
    $postRemoveId = $_REQUEST['removeId'];
    $adminQuery = $db->prepare('SELECT * FROM users WHERE user_id=:user_id and isadmin=1 LIMIT 1;');
    $adminQuery->execute([
        ':user_id' => $_SESSION['user_id']
    ]);
    if ($adminQuery->rowCount() > 0) {
        $postRemove = $db ->prepare('DELETE FROM posts WHERE post_id=:post_id;');
        $postRemove ->execute([
            'post_id' => $postRemoveId
        ]);
        header('Location: index.php?operationStatus=ok');
    } else {
        $postRemove=$db->prepare('SELECT * FROM posts WHERE user_id=:user_id and post_id=:post_id LIMIT 1;');
        $postRemove ->execute([
           ':user_id' => $_SESSION['user_id'],
           'post_id' => $postRemoveId
        ]);
        if($postRemove->rowCount()>0){
            $postRemove = $db ->prepare('DELETE FROM posts WHERE post_id=:post_id;');
            $postRemove ->execute([
                'post_id' => $postRemoveId
            ]);
            ...
            header('Location:index.php?failStatus=yes');
        }
    }
}
